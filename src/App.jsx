...
import { ArticlesFiltered } from "./components/ArticlesFiltered";
...
export default function App() {
  ...
  async function renderFilteredHotels() {
    try {
      ...
      if (!location || !numGuests) return;

      const result = allHotels.filter(
        (hotel) =>
          hotel.city.toLowerCase() === location.city.toLowerCase() &&
          hotel.country.toLowerCase() === location.country.toLowerCase() &&
          hotel.maxGuests >= numGuests
      );
  ...
  return (
    <>
      <div className="bg-white dark:bg-slate-900 dark:text-slate-200 transition-colors duration-300">
        <header
          id="navbar"
          ...
          <SerchBar numGuests={numGuests} location={location} setBtnPress={setBtnPress} btnPress={btnPress} setLocation={setLocation} setNumGuests={setNumGuests} />
        </header>
        <main className="w-full mt-40 md:mt-20 mb-10 md:p-5">
          ...
          <section className="my-10 mx-4 flex items-center justify-between md:my-5 md:mb-8">
            <h1 className="text-2xl font-bold text-gray-700 dark:text-slate-200">
              Stays in Finland
            </h1>
            <p id="numberOfCards" className="text-sm font-light text-gray-400">
              {filteredHotels.length > 0 ? (
            <p>{filteredHotels.length}+ stays</p>
          ) : (
            <p>12+ stays</p>
          )}
            </p>
          ...
          {/* <!-- Articulos --> */}
          {filteredHotels.length > 0 ? (
            <ArticlesFiltered filteredHotels={filteredHotels} />
          ) : (
            <Articles />
          )}
        </main>
        <footer className="w-full h-30 bg-slate-800 dark:bg-slate-800 text-slate-400 flex flex-col items-center justify-center p-5 gap-2">
          ...
          <div className="w-full flex items-center justify-center gap-2 text-xs">
            ...
            <a
              href="https://github.com/TITO9538/Proyecto-final-Funval-N1"
